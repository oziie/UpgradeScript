...
#FTP bağlantısında oluşturulacak klasör adı tanımlanıyor.
new_version = "Staj"

#FTP ve SSH bağlantısının yapılacağı makineler için bağlantı bilgileri tanımlanıyor.
hosts = [
    ##{'address' :"192.168.30.157",'user':'staj','passwd':'00590059' },
    {'address' :"192.168.30.156",'user':'uzemtest','passwd':'00590059' },
]

#Diğer metodların çalışmasını sağlayan ana metodtur.(FTP Connection,FTP Upload,SSH Connection,SSH Upgrade)
...
#SSH bağlantısının yapıldığı metodtur.
...
#SSH bağlantısı sağlandıktan sonra uzak makinede terminal açılıyor ve gerekli komutlar çalıştırılıyor.
def ssh_upgrade(ssh):
...
#FTP bağlantısını sağlayan metodtur.
...
#FTP ile gerekli dosyanın aktarılmasını sağlayan metodtur.
...
#Birden fazla makine için "thread" yapısı oluşturuluyor.
#Bir metod ve onun kullanacağı bilgiler tanımlanarak her bir makine için aynı sürecin yapılması sağlanıyor.
threads = []
for item in hosts:
